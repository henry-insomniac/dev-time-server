# dev-time-server Agent Guide

## 项目概述

`dev-time-server`：这是一个关于研发时间应用的服务
...
- `.claude/dev-time-agent-architecture.md`：Agent Runtime 服务的定位、边界、工作流、数据契约和安全约束。
...
- 涉及产品定位、MVP、风险模型、Agent 场景、Agent 服务边界或跨端技术架构时，同步更新 `dev-time`、`dev-time-server` 和 `dev-time-agent` 三个仓库中的 `.claude/product-prd.md` 与 `.claude/technical-architecture.md`。
